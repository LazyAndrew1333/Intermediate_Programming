...
public class app {
	...
	static int		lineToEdit 		= 10;
	static String 	input 			= "";
	static String 	log 			= "";
	static String 	fileName 		= "History.txt";
	static String	javaFileName	= "C:\\Users\\John Andrew\\OneDrive - National University\\Eclipse\\eclipse-workspace(onsite)\\Long Exam (January)\\src\\app.java";
	...
	static void checkInputForNumber(String str) {		// Called from line 88, the value of the parameter sent comes from the value of input.
		...
		// given input.
	static long factorial(long n) {						// Called from line 106, the value of the parameter sent comes from the length of the input.
		...
		// in the history log.
	static void outputLogger(long result, String str) throws IOException {								// Called from line 110, the value of the first parameter(result) comes from the factorial if the input length. The second parameter(str) comes from the value of the user input.
		if (result == 1) {																					// if the result is equal to 1, it returns "permutation" (without 's')
			log = "Attempt "+ attempt + " : (" + now + ")\n" + str + " have " + result + " permutation\n" + str + ": ";
		} else {																							// else if the result is greater than 1, it returns "permutations" (with 's')
			log = "Attempt "+ attempt + " : (" + now + ")\n" + str + " have " + result + " permutations\n" + str + ": ";
		}
	}
	
		// This method is used in order to get all of the
		// distinct combination/permutation of the given input.
	static void printDistinctPermutn(String str, String ans) {		// Called from line 109. The value of the first parameter(str) comes from the input. The value of the second parameter(ans) is an empty string which will be modified by the method.
		if (str.length() == 0) {										// After the ros goes through serial combinations, the length of str becomes zero
            log = log + ans + " ";      								// in which the gathered combinations stored in ans is then added to the log.
            return;
        }
	...
	static void appendStrToFile(String str) {						// Called from line 128. The value of the parameter comes from the input.
		...
		// attempts.
	static void attemptCounter() {														// Called from line 155. This method is called first so that it could adjust the attempt count before the programs starts making logs.
		try {
			...
			BufferedReader reader = new BufferedReader(new FileReader(javaFileName));		// Reads the java file line by line.
			String line;
	...
	public static void main(String[] args) {
		...
		try {
			attemptCounter();
			outputLogger(factorial(input.length()), input);
			printDistinctPermutn(input , "");
			System.out.println(log);
			
			appendStrToFile(input);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
